...
void forward_propagation(int n_total, int n_input, int n_output, int *input_array, struct Graph *graph)
{
    ...
    memset(ans,0,maxn*sizeof(int));//初始化存储所需时间的数组
    for(int i=1;i<=n_input;i++){//为输入层神经节赋权值
        ans[i] = input_array[i];
    }
    for(int i=1;i<=n_total;i++){//按顺序对每个节点
        if(graph->head[i] == -1)//无出度
            break;
        for(nowEdge = &(graph->edegs[graph->head[i]]);; nowEdge = &(graph->edegs[nowEdge->next])){//沿着每条边加入贡献值
